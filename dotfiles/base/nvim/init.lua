...
do
  ...
  -- (the picker is set up in the next section).
  ...
  if vim.g.have_nerd_font then
    ...
    -- Used for backwards compatibility with plugins that still require `nvim-web-devicons`
...
-- snacks.nvim: picker (fuzzy finding) and explorer (file tree)
-- ============================================================
do
  -- [[ snacks.nvim: picker + explorer ]]
  --
  -- snacks.nvim is a collection of small quality-of-life modules ("snacks").
  -- We enable only the ones we need here:
  --
  --  * picker:   a fuzzy finder. It's more than just a "file finder", it can
  --              search many different aspects of Neovim, your workspace, LSP,
  --              and more! It also takes over `vim.ui.select`, so any plugin
  --              that asks you to pick from a list automatically uses it.
  --  * explorer: a file tree sidebar. With `replace_netrw = true` (the default,
  --              written out below so you know where to look) it also takes over
  --              netrw, so `nvim .` or `:edit .` opens the explorer instead.
  --  * bigfile / quickfile: small performance helpers that make opening very
  --              large files snappy. No keymaps or behavior to learn.
  --
  -- The easiest way to learn the picker is to open one (try `<leader>sf`) and
  -- press `?` while focused on the list: that shows all keymaps for the
  -- current picker. Run `<leader>ss` to fuzzy find every available picker.
  ...
  require('snacks').setup {
    ...
    picker = {
      -- ui_select = true is the default: `vim.ui.select` uses the picker.
    },
    explorer = {
      -- Take over netrw as the default file browser (this is the default,
      -- shown explicitly so you know where to change it).
      replace_netrw = true,
    },
  }
  ...
  -- All picker functions live on `Snacks.picker`. Run `:lua =vim.tbl_keys(Snacks.picker)`
  -- (or `<leader>ss`) to discover everything it can search.
  ...
  -- Live grep, but only in the files you currently have open.
...
do
  ...
  --  (No file tree example here: the snacks explorer from Section 5 already covers that.)
